...
if (tool === 'artify') { return toolState.selectedStyle + preservationPrompt; }
...
editControls = Object.entries(styles).map(([category, styleList]) => <div class="mb-6"><h3 class="style-category-title">${category}</h3><div class="style-grid">${styleList.map(s => ${s.name}).join('')}</div></div>).join('');
...
let resultHTML = '';
if (state.activeTool === 'image-series') { resultHTML = <div class="style-grid">${toolState.resultUrls.map(url => <img src="${url}">).join('')}</div>; }
