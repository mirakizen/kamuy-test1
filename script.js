document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateImage = async () => {
        ...
        try {
            const isTextToImage = state.activeTool === 'image-series'; // This will be used later
            const endpoint = isTextToImage ? '/api/series' : '/api/edit';
            
            let body = { prompt: toolState.engineeredPrompt };
            if (!isTextToImage) {
                const compressedFile = await compressImage(toolState.file);
                const base64Image = await fileToBase64(compressedFile);
                body = { ...body, image: base64Image, imageName: compressedFile.name, imageType: compressedFile.type, width: toolState.dimensions.width, height: toolState.dimensions.height };
            }
    ...
    const render = () => {
        ...
        if (toolState.view === 'upload') {
            ...
            if (state.activeTool === 'prompt-edit') { editControls = `<div><label for="prompt-input" class="block text-sm font-bold mb-2">Describe your edit:</label><textarea id="prompt-input" rows="3" class="w-full p-2 border border-primary rounded-md bg-transparent" placeholder="e.g., add sunglasses to the corgi">${toolState.userInput || ''}</textarea></div>`; }
                ...
                const styles = {
                    'Master Artists': [ { name: 'Van Gogh', prompt: 'in the style of Van Gogh, expressive impasto brushstrokes'}, { name: 'Monet', prompt: 'in the style of Monet, impressionism, soft light' }, { name: 'Picasso', prompt: 'in the cubist style of Picasso, abstract, geometric' } ],
                    'Digital & Retro': [ { name: 'Cyberpunk', prompt: 'cyberpunk style, neon lights, futuristic city, cinematic lighting' }, { name: 'Pixel Art', prompt: '16-bit pixel art style' }, { name: 'Voxel Art', prompt: 'voxel art, isometric, blocky, 3d pixels' } ],
                    'Handcrafted & Physical': [ { name: 'LEGO', prompt: 'as a LEGO diorama, plastic brick texture' }, { name: 'Pencil Sketch', prompt: 'pencil sketch, hand-drawn, monochrome' }, { name: 'Claymation', prompt: 'claymation style, plasticine, stop-motion look' } ]
                };
